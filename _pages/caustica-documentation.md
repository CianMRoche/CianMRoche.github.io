...
- **`computeDiscImageOutlines(planes, dist, sourceIdx, source, fov, gridN, grid?)`**: extracts the uniform-disc lensed-image boundary (the zero level-set of $r_\text{ell}(\boldsymbol{\beta}(\boldsymbol{\theta})) - r$) by marching squares over the same $\boldsymbol{\beta}$ grid, with per-vertex secant refinement so high-magnification arcs stay smooth (§7, Line art).
...
- **`drawLineArt()`**: the line-art render path (§7, Line art). When `state.lineArt` is set, `_doRedraw()` skips the GPU pass and `drawOverlay()` delegates here; it fills the overlay with the palette background (covering the GL canvas), draws the uniform-disc lensed-image outlines (`computeDiscImageOutlines` → `chainSegments` → `smoothPolylines`), the point-source image dots, and the critical curves / caustics in the palette colours. Palettes live in `LINE_ART_PALETTES`; `buildCompositeCanvas()` returns the overlay alone for PNG/recording capture in this mode.
